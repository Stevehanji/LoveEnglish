...
class TextInputBox:
    def __init__(self, x, y, width, height, length = 29):
        self.x = x
        self.y = y
        self.w = width
        self.h = height
        self.manager = TextInputManager(validator = lambda input : len(input) <= length)
        self.textinput = TextInputVisualizer(manager=self.manager)
        self.Is_Input = False
    
    def SetFont(self, size, font, color = "black"):
        Font = pygame.font.Font(font, size)
        self.textinput.font_object = Font
        self.textinput.font_color = color
    
    def draw(self, event):
        Box = pygame.draw.rect(screen, "white", (self.x, self.y, self.w, self.h))

        if BoxButton.Is_Clicked(Box):
            self.Is_Input = True
        
        # print(str(self.textinput.surface))
        if self.Is_Input == True:
            self.textinput.update(event)
        
        screen.blit(self.textinput.surface, (self.x + 10, self.y + self.h // 2 - self.textinput.surface.get_height() // 2))

    # def adjust_value(self, value, event):
    ...
    def Reset_Button_Box(self, event):
        self.adjust_value("", event)
        self.Is_Input = False
    
    def showCurso(self, show = True):
        self.textinput.cursor_visible = show
